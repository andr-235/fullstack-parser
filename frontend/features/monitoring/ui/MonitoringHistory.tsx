...
export default function MonitoringHistory({ }: MonitoringHistoryProps) {
 ...
 const successRate = Math.round((mockData.successfulRuns / mockData.totalRuns) * 100)
  ...
  <div className="space-y-6">
   ...
   <div className="flex items-center justify-between">
    <div className="flex items-center space-x-3">
     ...
     <div>
      <h2 className="text-xl font-bold text-slate-200">История мониторинга</h2>
     ...
     <CardContent className="p-4">
      <div className="flex items-center justify-between">
       <div>
        ...
        <p className="text-2xl font-bold text-blue-400">{mockData.totalRuns}</p>
       </div>
     ...
     <CardContent className="p-4">
      <div className="flex items-center justify-between">
       <div>
        ...
        <p className="text-2xl font-bold text-green-400">{mockData.successfulRuns}</p>
        <p className="text-xs text-slate-400">{successRate}% успешность</p>
       </div>
     ...
     <CardContent className="p-4">
      <div className="flex items-center justify-between">
       <div>
        ...
        <p className="text-2xl font-bold text-red-400">{mockData.failedRuns}</p>
        <p className="text-xs text-slate-400">
         {Math.round((mockData.failedRuns / mockData.totalRuns) * 100)}% ошибок
        </p>
       </div>
     ...
     <CardContent className="p-4">
      <div className="flex items-center justify-between">
       <div>
        ...
        <p className="text-2xl font-bold text-purple-400">{mockData.averageDuration}с</p>
      ...
      <CardContent>
       <div className="space-y-4">
        <div className="text-center py-8">
         ...
         <p className="text-slate-400 font-medium">График активности</p>
         <p className="text-sm text-slate-500 mt-1">
          Детальная визуализация будет добавлена в следующем обновлении
         </p>
        </div>
       </div>
      </CardContent>
       ...
       <div className="space-y-4">
        {mockData.recentErrors.length > 0 ? (
         mockData.recentErrors.map((error) => (
          <div key={error.id} className="p-4 bg-slate-700 rounded-lg border-l-4 border-red-500">
           <div className="flex items-start justify-between">
            <div className="flex-1">
             <div className="flex items-center space-x-2 mb-2">
              <AlertTriangle className="h-4 w-4 text-red-400" />
              <span className="font-medium text-slate-200">{error.groupName}</span>
             </div>
             <p className="text-sm text-slate-300 mb-2">{error.error}</p>
             <p className="text-xs text-slate-400">
              {format(error.timestamp, 'dd.MM.yyyy HH:mm', { locale: ru })}
             </p>
            </div>
           </div>
          </div>

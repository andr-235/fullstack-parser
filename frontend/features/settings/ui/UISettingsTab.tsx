...
export function UISettingsTab() {
  ...
  const handleInputChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [field]:
        field === 'theme' ? (value as 'light' | 'dark' | 'system') : value,
    }))
  }
  ...
  const isRefreshIntervalValid =
    formData.refresh_interval >= SETTINGS_VALIDATION.ui.refresh_interval.min &&
    formData.refresh_interval <= SETTINGS_VALIDATION.ui.refresh_interval.max

  const isItemsPerPageValid =
    formData.items_per_page >= SETTINGS_VALIDATION.ui.items_per_page.min &&
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500">Цветовая схема интерфейса</p>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                handleInputChange('auto_refresh', checked)
              }
            />
          </div>
          ...
          {formData.auto_refresh && (
            <div className="space-y-2">
              <Label
                htmlFor="refresh_interval"
                className="flex items-center gap-2"
              >
                Интервал обновления (секунды)
                <Badge
                  variant={isRefreshIntervalValid ? 'default' : 'destructive'}
                >
              ...
              />
              <p className="text-xs text-slate-500">
                Как часто обновлять данные (от{' '}
                {SETTINGS_VALIDATION.ui.refresh_interval.min} до{' '}
                {SETTINGS_VALIDATION.ui.refresh_interval.max} сек)
              </p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                handleInputChange('show_notifications', checked)
              }
            />
          </div>

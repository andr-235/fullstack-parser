...
export function DatabaseSettingsTab() {
  ...
  const isPoolSizeValid =
    formData.pool_size >= SETTINGS_VALIDATION.database.pool_size.min &&
    formData.pool_size <= SETTINGS_VALIDATION.database.pool_size.max

  const isMaxOverflowValid =
    formData.max_overflow >= SETTINGS_VALIDATION.database.max_overflow.min &&
    formData.max_overflow <= SETTINGS_VALIDATION.database.max_overflow.max

  const isPoolRecycleValid =
    formData.pool_recycle >= SETTINGS_VALIDATION.database.pool_recycle.min &&
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                handleInputChange('pool_size', parseInt(e.target.value) || 10)
              }
            />

...
import { useSettings, useUpdateSettings, useTestVKAPIConnection } from '@/hooks/use-settings'
...
export function VKAPISettingsTab() {
 ...
 const isRequestsValid = formData.requests_per_second >= SETTINGS_VALIDATION.vk_api.requests_per_second.min &&
  formData.requests_per_second <= SETTINGS_VALIDATION.vk_api.requests_per_second.max
  ...
  <div className="space-y-6">
   ...
   <Card>
    ...
    <CardContent className="space-y-4">
     ...
     {/* Requests per Second */}
     <div className="space-y-2">
      <Label htmlFor="requests_per_second" className="flex items-center gap-2">
       Запросов в секунду
      ...
      <Input
       ...
       onChange={(e) => handleInputChange('requests_per_second', parseInt(e.target.value) || 3)}
      />
      <p className="text-xs text-slate-500">
       Лимит запросов в секунду (от {SETTINGS_VALIDATION.vk_api.requests_per_second.min} до {SETTINGS_VALIDATION.vk_api.requests_per_second.max})
      </p>
     </div>
     ...
     <div className="flex gap-3 pt-4">
      <Button
       onClick={handleSave}
       disabled={updateSettings.isPending || !isTokenValid || !isRequestsValid}
      >
      ...
      >
       ...
       {testConnection.isPending ? 'Тестирование...' : 'Тест подключения'}
      </Button>
     </div>
    </CardContent>
   </Card>
   ...
   <Card>
    ...
    <CardContent>
     <div className="space-y-2 text-sm">
      <p>• Access Token можно получить в настройках приложения ВКонтакте</p>

...
export function MonitoringSettingsTab() {
 ...
 if (settingsData && !isLoading && formData.scheduler_interval_seconds === 300) {
  setFormData({
   scheduler_interval_seconds: settingsData.settings.monitoring.scheduler_interval_seconds,
   max_concurrent_groups: settingsData.settings.monitoring.max_concurrent_groups,
   group_delay_seconds: settingsData.settings.monitoring.group_delay_seconds,
   auto_start_scheduler: settingsData.settings.monitoring.auto_start_scheduler,
  })
 }
 ...
 const isIntervalValid = formData.scheduler_interval_seconds >= SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.min &&
  formData.scheduler_interval_seconds <= SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.max

 const isGroupsValid = formData.max_concurrent_groups >= SETTINGS_VALIDATION.monitoring.max_concurrent_groups.min &&
  formData.max_concurrent_groups <= SETTINGS_VALIDATION.monitoring.max_concurrent_groups.max

 const isDelayValid = formData.group_delay_seconds >= SETTINGS_VALIDATION.monitoring.group_delay_seconds.min &&
  formData.group_delay_seconds <= SETTINGS_VALIDATION.monitoring.group_delay_seconds.max
   ...
   <Card>
    ...
    <CardContent className="space-y-4">
     ...
     <div className="space-y-2">
      <Label htmlFor="scheduler_interval" className="flex items-center gap-2">
      ...
      <Input
       ...
       type="number"
       min={SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.min}
       max={SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.max}
       value={formData.scheduler_interval_seconds}
       onChange={(e) => handleInputChange('scheduler_interval_seconds', parseInt(e.target.value) || 300)}
      />
      <p className="text-xs text-slate-500">
       Как часто запускать цикл мониторинга (от {SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.min} до {SETTINGS_VALIDATION.monitoring.scheduler_interval_seconds.max} сек)
      </p>
     </div>
     ...
     <div className="space-y-2">
      ...
      <Input
       ...
       onChange={(e) => handleInputChange('max_concurrent_groups', parseInt(e.target.value) || 10)}
      />
      <p className="text-xs text-slate-500">
     ...
     <div className="space-y-2">
      ...
      <Input
       ...
       onChange={(e) => handleInputChange('group_delay_seconds', parseInt(e.target.value) || 1)}
      />
      <p className="text-xs text-slate-500">
     ...
     <div className="flex items-center justify-between">
      ...
      <Switch
       ...
       onCheckedChange={(checked) => handleInputChange('auto_start_scheduler', checked)}
      />
     </div>
     ...
     <div className="flex gap-3 pt-4">
      <Button
       onClick={handleSave}
       disabled={updateSettings.isPending || !isIntervalValid || !isGroupsValid || !isDelayValid}
      >

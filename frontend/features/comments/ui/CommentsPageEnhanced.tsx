...
import { useBulkMarkAsViewed, useBulkArchive, useBulkUnarchive, useBulkDelete } from '@/entities/comment'
...
export default function CommentsPageEnhanced() {
 ...
 const apiFilters = useMemo(() => {
  ...
  if (filters.authorScreenName.length > 0) params.author_screen_name = filters.authorScreenName
 ...
 const handleArchiveComment = async (commentId: number) => {
  ...
  try {
   ...
   setLoadingComments(prev => ({ ...prev, [`archive-${commentId}`]: false }))
  }
 }

 const handleUnarchiveComment = async (commentId: number) => {
  setLoadingComments(prev => ({ ...prev, [`unarchive-${commentId}`]: true }))
  try {
   await unarchiveMutation.mutateAsync(commentId)
  } finally {
   setLoadingComments(prev => ({ ...prev, [`unarchive-${commentId}`]: false }))
  }
 }
 ...
 const stats = useMemo(() => {
  if (!commentsData?.pages) return { total: 0, new: 0, viewed: 0, archived: 0, withKeywords: 0 }
  ...
  return {
   ...
   withKeywords: allComments.filter(c => c.matched_keywords_count > 0).length,
  }
 }, [commentsData])

 const allComments = commentsData?.pages?.flatMap(page => page.items) || []
 const isProcessing = bulkMarkAsViewed.isPending || bulkArchive.isPending ||
  bulkUnarchive.isPending || bulkDelete.isPending

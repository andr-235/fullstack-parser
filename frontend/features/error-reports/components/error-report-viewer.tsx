...
interface ErrorEntry {
 timestamp: string
 error_type: 'validation' | 'database' | 'api' | 'network' | 'authentication' | 'authorization' | 'rate_limit' | 'timeout' | 'unknown'
...
export function ErrorReportViewer({ report, onAcknowledge, onDismiss }: ErrorReportViewerProps) {
   ...
   <CardContent className="space-y-6">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
     <div className="text-center p-3 bg-muted rounded-lg">
      <div className="text-2xl font-bold text-red-600">{report.total_errors}</div>
     ...
     {report.groups_processed !== undefined && (
      <>
       <div className="text-center p-3 bg-muted rounded-lg">
        <div className="text-2xl font-bold text-blue-600">{report.groups_processed}</div>
        <div className="text-sm text-muted-foreground">Обработано</div>
       </div>
       <div className="text-center p-3 bg-muted rounded-lg">
        <div className="text-2xl font-bold text-green-600">{report.groups_successful}</div>
        <div className="text-sm text-muted-foreground">Успешно</div>
       </div>
       <div className="text-center p-3 bg-muted rounded-lg">
        <div className="text-2xl font-bold text-orange-600">{report.groups_failed}</div>
    ...
    {report.recommendations.length > 0 && (
     <Alert>
      ...
      <AlertDescription>
       <div className="font-medium mb-2">Рекомендации по исправлению:</div>
      ...
      <Collapsible key={index}>
       ...
       <CollapsibleContent className="px-4 pb-4">
        <div className="space-y-3">
         ...
         {error.stack_trace && (
          <div>
           <div className="font-medium text-sm mb-1">Stack Trace:</div>
           <div className="text-xs text-muted-foreground bg-muted p-3 rounded max-h-40 overflow-auto">
            <pre className="whitespace-pre-wrap">{error.stack_trace}</pre>
           </div>
          </div>
         )}
        </div>
    ...
    <div className="flex justify-end gap-2">
     {onDismiss && (
      <Button variant="outline" onClick={() => onDismiss(report.report_id)}>
       Закрыть
      </Button>
     )}
     {onAcknowledge && (

#!/usr/bin/env python3
"""
Тест для парсинга с очень маленьким количеством постов
"""
...
async def test_small_parsing():
    """Тестирует парсинг с очень маленьким количеством постов"""
    print("🔍 Тестируем парсинг с очень маленьким количеством постов...")
    ...
    async with AsyncSessionLocal() as db:
        ...
        print(f"\n📋 Тест: Парсинг группы {group_id} (только 10 постов)")
        try:
            # Запускаем парсинг с очень маленьким количеством постов
            result = await parser_service.parse_group_posts(
                group_id=group_id,
                max_posts_count=10,  # Только 10 постов
                force_reparse=False,
            )
...
if __name__ == "__main__":
    asyncio.run(test_small_parsing())

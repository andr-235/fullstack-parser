...
class Group(Base):
    ...
    vk_id = Base.Column(Base.Integer, unique=True, index=True, nullable=False)
    name = Base.Column(Base.String(255), nullable=False)
    screen_name = Base.Column(Base.String(255), unique=True, index=True)
    description = Base.Column(Base.Text)
    members_count = Base.Column(Base.Integer, default=0)
    is_active = Base.Column(Base.Boolean, default=True)

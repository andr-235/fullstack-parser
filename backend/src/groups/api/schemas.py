...
class GroupCreate(BaseModel):
    ...
    vk_id: PositiveInt = Field(..., description="ID группы в VK (положительное целое число)")
    screen_name: Annotated[str, Field(min_length=1, max_length=255, pattern=r'^[a-zA-Z0-9_.]+


class GroupUpdate(BaseModel):
    """Схема для обновления группы"""
    name: Optional[Annotated[str, Field(min_length=1, max_length=255)]] = Field(None, description="Новое название группы")
    screen_name: Optional[Annotated[str, Field(min_length=1, max_length=255, pattern=r'^[a-zA-Z0-9_.]+
...
, description="Короткое имя группы (только буквы, цифры, точки и подчеркивания)")]
    name: Annotated[str, Field(min_length=1, max_length=255, description="Название группы")]
    description: Optional[Annotated[str, Field(max_length=1000)]] = Field(None, description="Описание группы (максимум 1000 символов)")

    @field_validator('screen_name')
    @classmethod
    def validate_screen_name(cls, v: str) -> str:
        """Валидация screen_name: не должно начинаться с цифры"""
        if v[0].isdigit():
            raise ValueError('screen_name не может начинаться с цифры')
        return v.lower()
...
)]] = Field(None, description="Новое короткое имя")
    description: Optional[Annotated[str, Field(max_length=1000)]] = Field(None, description="Новое описание")
    is_active: Optional[bool] = Field(None, description="Активировать/деактивировать группу")

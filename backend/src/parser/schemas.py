...
class ParseResponse(BaseModel):
    ...
    estimated_time: Optional[int] = Field(
        None, description="Ожидаемое время выполнения в секундах"
    )
...
class ParseStatus(BaseModel):
    ...
    groups_completed: int = Field(
        ..., description="Количество завершенных групп"
    )
...
class ParserState(BaseModel):
    ...
    total_tasks_processed: int = Field(
        ..., description="Всего обработано задач"
    )

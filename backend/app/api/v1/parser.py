...
from app.schemas.parser import ParseTaskCreate, ParseTaskResponse, ParseStats, GlobalStats, DashboardStats
from app.schemas.vk_comment import VKCommentResponse, CommentWithKeywords, CommentSearchParams
...
async def run_parsing_task(task_id: str, group_id: int, max_posts: Optional[int] = None):
...
async def get_comments(
    ...
    if search_params.group_id:
        query = query.join(VKComment.post).where(VKComment.post.has(group_id=search_params.group_id))
...
async def get_global_stats(
    ...
    last_parse_result = await db.execute(
        select(func.max(VKGroup.last_parsed_at)).where(VKGroup.last_parsed_at.isnot(None))
    )

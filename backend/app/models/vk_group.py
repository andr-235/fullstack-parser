...
class VKGroup(BaseModel):
    ...
    # Основная информация
    vk_id = Column(Integer, unique=True, nullable=False, index=True, comment="ID группы в ВК")
    screen_name = Column(String(100), nullable=False, comment="Короткое имя группы (@group_name)")
    ...
    max_posts_to_check = Column(Integer, default=100, comment="Максимум постов для проверки")
    
    # Статистика
    last_parsed_at = Column(DateTime, comment="Когда последний раз парсили группу")
    total_posts_parsed = Column(Integer, default=0, comment="Общее количество обработанных постов")
    total_comments_found = Column(Integer, default=0, comment="Общее количество найденных комментариев")

...
from .database import get_async_engine, AsyncSessionLocal
...
class DatabaseService:
    ...
    async def get_by_id(
        ...
        try:
            ...
            logger.error("Get by ID error", model=model.__name__, id=id, error=str(e))
            raise DatabaseError(f"Failed to get {model.__name__} by ID: {str(e)}")
    ...
    async def get_many_by_ids(
        ...
        try:
            ...
            logger.error("Get many by IDs error", model=model.__name__, ids=ids, error=str(e))
            raise DatabaseError(f"Failed to get {model.__name__} by IDs: {str(e)}")
    ...
    async def bulk_create(
        ...
        try:
            ...
            logger.error("Bulk create error", model=model.__name__, count=len(data_list), error=str(e))
            raise DatabaseError(f"Failed to bulk create {model.__name__}: {str(e)}")
    ...
    async def update(
        ...
        try:
            ...
            logger.error("Update error", model=model.__name__, id=id, error=str(e))
            ...
            async with self.get_session() as session:
                ...
                for update_data in updates:
                    ...
                    if id_value is not None:
                        query = update(model).where(model.id == id_value).values(**update_data)
                        result = await session.execute(query)
                        total_updated += result.rowcount
                
                return total_updated
        except SQLAlchemyError as e:
            logger.error("Bulk update error", model=model.__name__, count=len(updates), error=str(e))
            raise DatabaseError(f"Failed to bulk update {model.__name__}: {str(e)}")
    ...
    async def delete(
        ...
        try:
            ...
            logger.error("Delete error", model=model.__name__, id=id, error=str(e))
    ...
    async def exists(
        ...
        try:
            ...
            raise DatabaseError(f"Failed to check existence of {model.__name__}: {str(e)}")

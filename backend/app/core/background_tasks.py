...
class BackgroundTaskManager:
    ...
    async def start(self):
        ...
        self.running = True
        logger.info(
            "Starting background task manager", max_workers=self.max_workers
        )
        ...
        **kwargs,
        ...
        if not self.running:
            raise ServiceUnavailableError(
                "Background task manager is not running"
            )
    ...
    async def _worker(self, worker_name: str):
        ...
        while self.running:
            try:
                ...
                logger.error(
                    "Worker error", worker_name=worker_name, error=str(e)
                )
    ...
    async def _process_task(self, task: BackgroundTask, worker_name: str):
        ...
        try:
            ...
            logger.error(
                "Task failed", task_id=task.id, name=task.name, error=str(e)
            )

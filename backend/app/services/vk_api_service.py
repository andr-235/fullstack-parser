...
class VKAPIService:
        ...
        try:
            ...
            if response:
                ...
                return {
                    ...
                    "photo_url": group.get("photo_200", group.get("photo_100", group.get("photo_50", "")))
                }
        except Exception as e:
            logger.error(f"Ошибка получения информации о группе {group_id}: {e}")
            return None
        
    async def get_group_posts(self, 
                             group_id: int, 
                             count: int = 100, 
        ...
        try:
            ...
            for item in response["items"]:
                ...
                if "profiles" in response:
                    author = self._find_author_info(item["from_id"], response.get("profiles", []), response.get("groups", []))
                    if author:
            ...
            logger.error(f"Ошибка получения комментариев поста {owner_id}_{post_id}: {e}")
            return []
    ...
    def _find_author_info(self, user_id: int, profiles: List[Dict], groups: List[Dict]) -> Optional[Dict]:
    ...
    async def get_comments_count(self, owner_id: int, post_id: int) -> int:
        ...
        try:
            ...
            logger.error(f"Ошибка получения количества комментариев {owner_id}_{post_id}: {e}")
            return 0 
...
class ParserService:
    ...
    async def parse_group_comments(self, group_id: int, max_posts: Optional[int] = None) -> Dict[str, int]:
        """
        ...
        posts = await self.vk_api.get_group_posts(group.vk_id, count=min(posts_limit, 100))
    ...
    async def _parse_post_comments(self, post: VKPost, keywords: List[Keyword]) -> Dict[str, int]:
        ...
        for comment_data in comments:
            try:
                ...
                matches = await self._find_keywords_in_text(comment_data["text"], keywords)
    ...
    async def _find_keywords_in_text(self, text: str, keywords: List[Keyword]) -> List[Tuple[Keyword, str, int]]:
        """
    ...
    async def _save_post(self, group: VKGroup, post_data: Dict) -> VKPost:
        ...
        result = await self.db.execute(
            select(VKPost).where(
                and_(
                    VKPost.vk_id == post_data["id"],
                    VKPost.group_id == group.id
                )
            )
        )
    ...
    async def _save_comment(self, post: VKPost, comment_data: Dict, matches: List[Tuple[Keyword, str, int]]) -> VKComment:
        ...
        for keyword, matched_text, position in matches:
            ...
            context_end = min(len(comment_data["text"]), position + len(matched_text) + 50)
